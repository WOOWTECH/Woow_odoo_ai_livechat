...
`odoo-ailivechat/` 目錄中提供了生產就緒的 `docker-compose.yml`，可使用 Odoo 18 和 PostgreSQL 16 快速部署。
...
- **網路：** 所有服務共用 `odoo-ailivechat-network` bridge 網路，允許內部主機名稱解析（例如 n8n 可透過 `http://odoo:8069` 存取 Odoo）。
...
4. 確認網路連線：`docker exec odoo-ailivechat-web curl -s http://db:5432 || echo "DB unreachable"`
